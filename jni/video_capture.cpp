...
static void init_mmap() {
	...
	if (req.count < 2) {
		ALOGE("Insufficient buffer memory on %s\n", dev_name);
		exit(EXIT_FAILURE);
	}
...
static void init_device(int width, int height) {
	...
	if (!(cap.capabilities & V4L2_CAP_STREAMING)) {
		ALOGE("%s does not support streaming i/o\n", dev_name);
		exit(EXIT_FAILURE);
	}
	CLEAR(fmt);
...
static void parse_im2(const unsigned char *im_yuv, unsigned char *dst, int width, int height) {
    const int IM_SIZE = width * height;
    unsigned char Y = 0;
    unsigned char U = 0;
    unsigned char V = 0;
...
static void parse_im3(const unsigned char *im_yuv, unsigned char *dst, int width, int height){
    memcpy(dst, im_yuv, width*height*2);
}
...
char video_capture(unsigned char* dst, int width, int height){
	...
	if(FD_ISSET(fd, &fds)){
        ...
        parse_im3(im_from_cam, dst, width, height);
